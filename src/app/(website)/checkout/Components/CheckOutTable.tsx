...
import { addQuantity, decrementQuantity, removeCart } from '@/app/redux_store/cartAddSlice';
const CheckOutTable = () => {
    const cart = useAppSelector((state)=>state?.cart?.products);
    const total = cart.reduce((sum,item)=>(sum + (item.discount * item.quantity)),0);
          ...
          <table className='table-auto w-[1400px] border-collapse  '>
            ...
            <tbody>
                ...
                <td className='border border-gray '>
                  ...
                  <div className='flex items-center justify-center'>
                    <div className='w-[124px] h-[138px] relative'>
                      ...
                      <Image src={item.images[0]} fill alt="product"/>
                    </div>
                    <p className='text-sea_green font-roboto text-[22px] font-normal '>{item.name}</p>

                  </div>

                  </div>
                 </td>
                  <td className='text-sea_green font-roboto text-[22px] font-normal border border-gray text-center'>{item.discount}</td>
                  <td className='border border-gray '>
                   <div className='flex items-center justify-center'>
                   <div className='flex items-center justify-between w-[171px] h-[51px] bg-very_light_green rounded-full p-1'>
                      <button disabled={item.quantity===1} onClick={()=>{dispatch(decrementQuantity(item.id))}} className='w-[44px] h-[43px] rounded-full border border-white bg-white'>-</button>
                      <p className='text-sea_green font-roboto text-[22px] font-medium'>{item.quantity}</p>
                      <button onClick={()=> {dispatch(addQuantity(item.id))}} className='w-[44px] h-[43px] rounded-full border border-white bg-white'>+</button>
                  ...
                  <td className='border border-gray'>
                    <button onClick={()=>{dispatch(removeCart(item.id))}} className='flex items-center justify-center px-10'>
              ...
              <tr>
                ...
                <td className='text-sea_green font-roboto text-[22px] font-medium text-center '>{total}</td>
                <td ></td>
              </tr>
            </tbody>
          </table>
